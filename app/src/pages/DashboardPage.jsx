  ...
  return (
    <Link to={`/politico/${col}/${p.id}`} className="ranking-card" style={{ display:'flex',alignItems:'center',gap:'12px',padding:'12px 16px',borderRadius:'var(--radius-sm)',background:'var(--bg-card)',border:'1px solid var(--border-light)',textDecoration:'none',color:'inherit' }}>
      ...
      <div style={{ textAlign:'right' }}>
        ...
        <div style={{ fontSize:'10px',color:'var(--text-secondary)' }}>indice</div>
      </div>
...
export default function DashboardPage({ user }) {
  ...
  return (
    <div className="container" style={{ maxWidth:'900px',margin:'0 auto',padding:'20px' }}>
      <h1 style={{ fontSize:'22px',fontWeight:700,marginBottom:'4px' }}>Painel de Fiscalizacao</h1>
      <p style={{ fontSize:'13px',color:'var(--text-secondary)',marginBottom:'16px' }}>
        {ok?`${filt.length} politicos`:''} &middot; Indice TransparenciaBR (Kim Kataguiri = 100)
      </p>
      ...
      {!ok ? (
        ...
        <>
          <div style={{ fontSize:'11px',color:'var(--text-secondary)',marginBottom:'16px',padding:'10px',background:'var(--bg-card)',borderRadius:'var(--radius-sm)',border:'1px solid var(--border-light)' }}>
            <strong>Base legal:</strong> Art. 37 CF/88, Lei 12.527/2011 (LAI). Dados publicos da Camara. Indice apartidario: economia (40%), processos (25%), presenca (20%), proposicoes (10%), defesas (5%).
          </div>

          {!loading && top10.length > 0 && (
            <div>
              <h2 style={{ fontSize:'17px',marginBottom:'4px' }}>Top 10 — Melhores Indices</h2>
              <p style={{ fontSize:'12px',color:'var(--text-secondary)',marginBottom:'12px' }}>Maior indice de transparencia, economia e atividade parlamentar.</p>
              <div style={{ display:'flex',flexDirection:'column',gap:'8px',marginBottom:'24px' }}>
                {top10.map((p,i) => <Card key={p.id} p={p} rank={i+1} col={colecao} top={true} />)}
              </div>

              <h2 style={{ fontSize:'17px',marginBottom:'4px' }}>Bottom 10 — Piores Indices</h2>
              <p style={{ fontSize:'12px',color:'var(--text-secondary)',marginBottom:'12px' }}>Menor indice. Dados publicos oficiais, sem juizo de valor.</p>
              <div style={{ display:'flex',flexDirection:'column',gap:'8px',marginBottom:'24px' }}>
          ...
          <h2 style={{ fontSize:'17px',marginBottom:'12px' }}>Ranking Completo</h2>
          {loading ? (
            <p style={{ textAlign:'center',padding:'40px' }}>Carregando...</p>
          ) : (
            <div style={{ display:'flex',flexDirection:'column',gap:'6px' }}>
              {filt.slice(0,100).map((p,i) => {
                const v = classificarScoreTransparenciaBR(p.idx);
                return (
                  <Link key={p.id} to={`/politico/${colecao}/${p.id}`} style={{ display:'flex',alignItems:'center',gap:'12px',padding:'10px 14px',borderRadius:'var(--radius-sm)',background:'var(--bg-card)',border:'1px solid var(--border-light)',textDecoration:'none',color:'inherit' }}>
                    ...
                    {p.idx!=null &&
                      <span style={{ fontWeight:700,fontSize:'18px',color:p.idx>=80?'var(--accent-green)':p.idx>=50?'var(--accent-orange)':'var(--accent-red)' }}>{p.idx}
                        <div style={{ fontSize:'10px',color:'var(--text-secondary)',fontWeight:400 }}>indice</div>
                      </span>
                    }
                    {p.idx!=null && <span className={v.className} style={{ fontSize:'11px',padding:'2px 8px',borderRadius:'4px' }}>{v.label}</span>}
                    <span style={{ color:'var(--text-secondary)',fontSize:'18px' }}>&rsaquo;</span>
                  </Link>
                );
              })}
            </div>
