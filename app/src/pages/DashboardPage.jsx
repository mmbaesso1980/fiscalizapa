...
function calcularIndiceTransparencia(p) {
  ...
  const temDados = totalGastos > 0 || totalDespesas > 0 || riskScore > 0;
  if (!temDados) return 30;
  const economiaScore = Math.max(0, 100 - Math.min((totalGastos / 600000) * 100, 100));
  const riskInvertido = Math.max(0, 100 - (riskScore * 1.5));
  const presencaScore = p.presencaScore || p.presenca || 50;
  const proposicoesScore = p.proposicoesScore || Math.min((totalDespesas > 20 ? 40 : totalDespesas > 0 ? 20 : 0) + 10, 50);
  const defesasScore = p.defesasPlenarioScore || 30;
  const raw = (economiaScore * 0.40) + (riskInvertido * 0.25) + (presencaScore * 0.20) + (proposicoesScore * 0.10) + (defesasScore * 0.05);
...
function normalizarPorKim(politicos) {
  ...
  if (!kim) return politicos.map(p => ({ ...p, scoreTransparencia: calcularIndiceTransparencia(p) }));
  const kimRaw = calcularIndiceTransparencia(kim);
  if (kimRaw <= 0) return politicos.map(p => ({ ...p, scoreTransparencia: calcularIndiceTransparencia(p) }));
  const fator = 100 / kimRaw;
  return politicos.map(p => {
    if (Number(p.idCamara) === KIM_ID_CAMARA) return { ...p, scoreTransparencia: 100 };
    const raw = calcularIndiceTransparencia(p);
    const normalizado = Math.round(Math.min(raw * fator, 100));
...
export default function DashboardPage({ user }) {
  ...
  const top10 = useMemo(() => {
    const withData = politicos.filter(p => (p.totalGastos || 0) > 0 || (p.totalDespesas || 0) > 0);
    const sorted = [...withData].sort((a, b) => (b.scoreTransparencia || 0) - (a.scoreTransparencia || 0));
    return sorted.slice(0, 10);
  }, [politicos]);
  const bottom10 = useMemo(() => {
    const withData = politicos.filter(p => (p.totalGastos || 0) > 0 || (p.totalDespesas || 0) > 0);
    const sorted = [...withData].sort((a, b) => (a.scoreTransparencia || 0) - (b.scoreTransparencia || 0));
    return sorted.slice(0, 10);
  }, [politicos]);
    ...
    <div className="page-container" style={{ maxWidth: 900, margin: '0 auto', padding: '20px' }}>
      ...
      {!isReady ? (
        <div style={{ textAlign: 'center', padding: '60px 20px', background: 'var(--bg-card)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-light)' }}>
          ...
          <p style={{ fontSize: '15px', color: 'var(--text-secondary)', maxWidth: 500, margin: '0 auto 16px' }}>Estamos coletando dados de <strong>{currentCol?.label}</strong>.</p>
        ...
        <>
          <div style={{ padding: '12px 16px', background: 'rgba(74,144,101,0.08)', borderRadius: 'var(--radius-sm)', marginBottom: '20px', border: '1px solid rgba(74,144,101,0.2)' }}>
            <p style={{ fontSize: '11px', color: 'var(--text-secondary)', lineHeight: 1.5 }}>
              <strong>Base legal:</strong> Art. 37 CF/88 (publicidade), Lei 12.527/2011 (LAI). Dados 100% publicos da Camara. Indice apartidario: economia de cota (40%), processos/risco (25%), presenca (20%), proposicoes (10%), defesas plenario (5%). Sem vinculacao ideologica.
            </p>
          </div>
          {!loading && top10.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(380px, 1fr))', gap: '20px', marginBottom: '32px' }}>
              <div>
                ...
                <p style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '10px' }}>Deputados com maior indice de transparencia e economia.</p>
              ...
              <div>
                ...
                <p style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '10px' }}>Deputados com menor indice. Dados publicos oficiais, sem juizo de valor.</p>
          ...
          {showMap && <MapaBrasil onSelectEstado={(estado) => setUf(estado || "")} politicoCounts={politicoCounts} />}
          ...
          {loading ? (
            <div style={{ textAlign: 'center', padding: '40px' }}><p style={{ color: 'var(--text-muted)' }}>Carregando...</p></div>
          ) : (
            <div>
              {filtered.slice(0, 100).map((p, i) => {
                ...
                return (
                  <Link to={`/politico/${colecao}/${p.id}`} key={p.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', background: 'var(--bg-card)', borderRadius: 'var(--radius-sm)', marginBottom: '8px', border: '1px solid var(--border-light)', textDecoration: 'none', color: 'inherit' }}>
                    ...
                    {p.scoreTransparencia != null && <span className={level.cls} style={{ padding: '4px 10px', borderRadius: '10px', fontSize: '11px', fontWeight: 600 }}>{level.label}</span>}
