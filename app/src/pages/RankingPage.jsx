...
export default function RankingPage() {
  ...
  const [totalFirestore, setTotal]= useState(0);
  ...
  // UFs e partidos únicos a partir dos dados reais
    ...
    <div style={{ minHeight: '100vh', background: '#FAFAF8', fontFamily: "'Inter', system-ui, sans-serif" }}>
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <p style={{ fontSize: 14, color: '#999', marginBottom: 0 }}>
            {loading ? 'Carregando...' : `${totalFirestore} deputados federais · índice baseado em dados públicos da Câmara dos Deputados`}
          </p>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 24, flexWrap: 'wrap' }}>
          ...
          <select
            ...
            style={{ flex: 1, minWidth: 110, padding: '10px 14px', borderRadius: 10, border: '1px solid #EDEBE8', fontSize: 14, background: '#fff', color: '#2D2D2D' }}
          >
            <option value="">Todos os estados ({ufs.length})</option>
            {ufs.map(uf => <option key={uf} value={uf}>{uf}</option>)}
          </select>
          <select
            value={filterPart}
            onChange={e => setFilterPart(e.target.value)}
            style={{ flex: 1, minWidth: 130, padding: '10px 14px', borderRadius: 10, border: '1px solid #EDEBE8', fontSize: 14, background: '#fff', color: '#2D2D2D' }}
          >
        ...
        {/* LISTA */}
        {loading ? (
          <div style={{ textAlign: 'center', padding: '64px', color: '#AAA', fontSize: 15 }}>
            Carregando {totalFirestore > 0 ? totalFirestore : ''} deputados...
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {filtered.map(dep => {
              ...
              return (
                <Link key={dep.id} to={`/politico/deputados/${dep.id_camara}`} style={{ textDecoration: 'none' }}>
                  <div
                    style={{
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 14, fontWeight: 600, color: '#2D2D2D', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {dep.nome}
                      </div>
                    ...
                    <div style={{ textAlign: 'right', flexShrink: 0, minWidth: 100 }}>
                      ...
                      <div style={{ fontSize: 13, fontWeight: 600, color: '#2D2D2D', fontVariantNumeric: 'tabular-nums' }}>
        ...
        {!loading && (
          <p style={{ marginTop: 24, fontSize: 12, color: '#CCC', textAlign: 'center' }}>
            {filtered.length} de {totalFirestore} deputados · Fonte: Dados Abertos da Câmara dos Deputados
          </p>
        )}
      </div>
    </div>
