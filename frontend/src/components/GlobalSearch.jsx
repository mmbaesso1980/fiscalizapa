...
export default function GlobalSearch() {
  ...
  return (
    <div className="relative w-full max-w-2xl z-50">
      {/* Live region for screen readers to announce search progress */}
        ...
        <div className="absolute top-12 left-0 w-full bg-slate-800 border border-slate-700 rounded-lg shadow-xl overflow-y-auto max-h-96">
          {results.map((item) => (
            <button
              ...
              className="w-full text-left flex items-center gap-3 p-3 hover:bg-slate-700 focus:bg-slate-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 cursor-pointer border-b border-slate-700/50 last:border-0 transition-colors"
            >
              {item.avatar_url ? (
