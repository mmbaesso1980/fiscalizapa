    ...
    <div className="relative w-full max-w-2xl z-50">
      <div className="relative">
        <input
          ...
          className="w-full bg-slate-900 border border-slate-700 text-sm rounded-full px-4 py-2 text-white focus:outline-none focus:border-blue-500 focus-visible:ring-2 focus-visible:ring-blue-500 transition-colors"
        />
        <span aria-hidden="true" className="absolute right-3 top-2 text-slate-400 text-sm">
          {isSearching ? '⏳' : '🔍'}
        </span>
      </div>
        ...
        <div className="absolute top-12 left-0 w-full bg-slate-800 border border-slate-700 rounded-lg shadow-xl overflow-y-auto max-h-96">
          {results.map((item) => (
            <button
              ...
              className="w-full text-left flex items-center gap-3 p-3 hover:bg-slate-700 focus-visible:bg-slate-700 focus-visible:outline-none cursor-pointer border-b border-slate-700/50 last:border-0 transition-colors"
            >
              {item.avatar_url ? (
