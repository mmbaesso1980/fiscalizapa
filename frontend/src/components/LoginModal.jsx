  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        {mode === "choose" ? (
          <>
            <button onClick={() => { onGoogle(); onClose(); }} style={btnStyle("#db4437")}>
              Entrar com Google
            </button>
            <button onClick={() => { onGitHub(); onClose(); }} style={btnStyle("#333")}>
              Entrar com GitHub
            </button>
            <button onClick={() => setMode("email")} style={btnStyle("#3d6b5e")}>
              Entrar com Email
            </button>
            <p style={{ textAlign: "center", fontSize: 13, marginTop: 12, color: "var(--text-secondary)" }}>
              Novo aqui?{" "}
              <span onClick={() => setMode("register")} style={{ color: "#3d6b5e", cursor: "pointer", fontWeight: 600 }}>
                Criar conta
              </span>
            </p>
          </>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <input type="password" placeholder="Senha (min 6 caracteres)" value={password}
            ...
            <p style={{ textAlign: "center", fontSize: 13, marginTop: 10, color: "var(--text-secondary)" }}>
              <span onClick={() => setMode(mode === "register" ? "email" : "register")}
                style={{ color: "#3d6b5e", cursor: "pointer", fontWeight: 600 }}>
                {mode === "register" ? "Ja tenho conta" : "Criar conta"}
              </span>
              {" | "}
              <span onClick={() => setMode("choose")} style={{ color: "#888", cursor: "pointer" }}>Voltar</span>
            </p>
          </form>
        )}
...
const inputStyle = {
  ...
  border: "1px solid #ccc", borderRadius: 8, fontSize: 14, boxSizing: "border-box"
};
