/**
 * fix-gastos-duplicados.js
 * Remove gastos com doc IDs tipo UUID (duplicatas) 
 * Mantém apenas os docs com ID determinístico (depId_ano_...)
 */
...
async function fixDeputado(depId) {
  ...
  if (snap.empty) return { depId, total: 0, uuids: 0, kept: 0 };
  ...
  for (const doc of snap.docs) {
    if (UUID_RE.test(doc.id)) {
      ...
      // ID determinístico — manter e somar
      const d = doc.data();
  ...
  // Atualizar o total de gastos no doc do deputado
  if (keptCount > 0) {
...
async function main() {
  ...
  for (const dep of depsSnap.docs) {
    ...
    try {
      ...
      totalKept += r.kept;
      if (r.uuids > 0) {
        console.log(`[${i}/${depsSnap.size}] ${dep.id}: ${r.total} docs → deletados ${r.uuids} UUIDs, mantidos ${r.kept} (R$ ${r.keptTotal})`);
      } else if (i % 50 === 0) {
        console.log(`[${i}/${depsSnap.size}] progresso...`);
      }
    } catch (e) {
      console.error(`ERRO ${dep.id}: ${e.message}`);
    }
  }
